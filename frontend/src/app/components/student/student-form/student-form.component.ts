...
export class StudentFormComponent implements OnInit {
  ...
  readonly studentColumnDefs: ColDef<StudentSubmission>[] = [
    ...
    },
    {
      ...
      maxWidth: 260,
      flex: 0,
      cellRenderer: (params: { data?: StudentSubmission }) => this.actionsCellRenderer(params.data)
    }
  ];
  ...
  private autoEditIfOwnRecord(): void {
    ...
    if (this.students.length === 1) {
      if (this.students[0].status === 'Pending') {
  ...
  // load boards
  loadBoards(): void {
    this.studentService.getBoards().pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
      next: (data) => {
        this.boards = data;
      },
      error: () => {
  ...
  // render the actions cell; only Pending applications may be edited
  private actionsCellRenderer(s?: StudentSubmission): string {
    ...
    const editButton = status === 'Pending'
      ...
      : '';
    return `<div class="grid-actions">${editButton}<button type="button" class="grid-action" data-action="view" title="View student details"><i class="bi bi-eye"></i><span>View</span></button><button type="button" class="grid-action delete" data-action="delete" title="Delete student"><i class="bi bi-trash3"></i><span>Delete</span></button></div>`;
  }
  ...
  startEdit(s: StudentSubmission): void {
    if ((s.status ?? 'Pending') !== 'Pending') {
      this.errorMessage = 'Only pending applications can be edited.';
      return;
    }
  ...
  deleteStudent(student: StudentSubmission): void {
    ...
    this.studentService.deleteSubmission(student.id).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
      next: () => {
  ...
  onBoardChange(): void {
    ...
    if (!boardId) return;
    this.studentService.getSessions(boardId).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
      next: (data) => {
        this.sessions = data;
      },
  ...
  onSessionChange(): void {
    ...
    this.studentService.getSchools(boardId, sessionId).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
      next: (data) => {
        this.schools = data;
      },
  ...
  onSchoolChange(): void {
    ...
    this.studentService.getClasses(boardId, sessionId, schoolId).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
      next: (data) => {
        this.classes = data;
      },
  ...
  onClassChange(): void {
    ...
    const classId = this.form.value.classId;
    if (!classId) return;
    this.studentService.getStreams(classId).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
      next: (data) => {
        this.streams = data;
      }
    });
    if (this.canHaveSpecialization) {
      this.studentService.getSpecializations(classId).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
        next: (data) => {
  ...
  onSubmit(): void {
    ...
    if (this.isEditing && this.editingId != null) {
      this.studentService.updateSubmission(this.editingId, student).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
        next: (updated) => {
      ...
      this.studentService.createSubmission(student).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
        next: () => {
  ...
  private resetForm(): void {
    this.form.reset({
      ...
      motherTongue: '', category: '', enrollmentNumber: '',
      termsAccepted: false,
